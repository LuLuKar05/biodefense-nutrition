...
================================================================
Handles:
  - /plan — Generate a one-day meal plan
...
async def cmd_plan(user_id: str, profile: dict[str, Any]) -> str:
    """Generate a new meal plan for the user."""
    # Check if there's already an active plan today
    active = get_active_plan(user_id)
    if active and active.get("status") == "accepted":
        return (
            "You already accepted a plan today! 🎯\n\n"
            f"{format_plan(active)}\n\n"
            "Use /regenerate if you want a completely new plan,\n"
            "or /log to record what you ate."
        )
    ...
    # Generate new plan
    plan = await generate_plan(user_id, profile)
...
def cmd_accept(user_id: str) -> str:
    ...
    return (
        "Plan accepted! ✅\n\n"
        "Now track what you actually eat:\n"
        "  /log Grilled chicken with brown rice for lunch\n"
...
async def cmd_log_meal(user_id: str, profile: dict[str, Any], meal_text: str) -> str:
    ...
    # Detect meal type from text or time
    meal_type = _detect_meal_type(meal_text)

    # Estimate macros from description
...
async def _handle_free_text(user_id: str, profile: dict[str, Any], text: str) -> str:
    ...
    return (
        ...
        "  /plan — Get a personalized meal plan\n"
...
async def _handle_chain(
    ...
    # Generate plan with threat context considered
    plan = await generate_plan(user_id, profile)

    lines = [
        f"⚠️ Adapting your meal plan for: {threat_type}",
    ]
    ...
    if boost_nutrients:
        lines.append(f"Boosting: {', '.join(boost_nutrients)}")
